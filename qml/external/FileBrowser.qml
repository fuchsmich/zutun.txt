...
Page {
    ...
    Component {
        ...
        Dialog {
            ...
            acceptDestination: pageStack.find(function(p){ return (p.name === "settings")})
            acceptDestinationAction: PageStackAction.Pop
            onAccepted: settings.todoTxtLocation = filePath;
            ...
            onClicked: {
                if (model.isDir) {
                    ...
                    if (mimeType.indexOf("text/") == 0) {
                        pageStack.push(confirmDialog, {filePath: filePath});
                    }
                }
            }
            onPressAndHold: {
                if (model.isDir) {
                    var filePath = Qt.resolvedUrl(fileModel.path + "/" + model.fileName + "/todo.txt");
                    pageStack.push(confirmDialog, {filePath: filePath});
                }
            }
        }
